...
function findLongestWord( array_words ){
	var result = 0;
	for (var i = 0; i <= array_words.length - 1; i++) {
		word_length = array_words[i].length;
		if (word_length > result) { 
			result = word_length;
		}
	}
	return result;
...
array_lengths.push( findLongestWord( ["One", "Two", "Three","Four","Five","Six","Seven"] ) );
array_lengths.push( findLongestWord( ["Epic", "German Railroads","Brains","Deep Sea Adventure"]) );
...
function arrangeElements( array )
{
	...
	for (var i = array.length - 1; i >= 0; i--) {
		if (Number.isFinite(array[i])) {
			temporal_array.unshift(array[i]);
		}
		else {
			temporal_array.push(array[i]);
		}
	}
...
function beautifyLetters( array )
{
	...
	for (var i = array.length - 1; i >= 0; i--) {
		if (["a","e","i","o","u","A","E","I","O","U"].includes(array[i])) {
			array[i] = array[i].toUpperCase();
		}
		else if (Number.isFinite(array[i])) {}
		else {
...
function beautifyNumbers( array )
{
	...
	for (var i = 0; i < array.length; i++) {
		if (Number.isFinite(array[i])) {
			while (array[i] > 9) {
				sum = 0;
				numbers_array = array[i].toString().split("").map(Number);
...
function sortArray( array )
{
	...
	for (var i = array.length - 1; i >= 0; i--) {
		if (Number.isFinite(array[i])) {
			numbers_array.push(array[i]);
		}
		else {
			letters_array.push(array[i]);
		}
	}
	numbers_array = numbers_array.sort(function(a, b){return a-b});
	letters_array = letters_array.sort();
	array = numbers_array.concat(letters_array);
	return(array);
}
